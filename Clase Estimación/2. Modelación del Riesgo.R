...
#Muestral
id.maximo<-which.max((dgamma(x = datos,shape = n,rate = lambda)))
datos[id.maximo]
...
chisq.test(x = brl, p = rep(x = 1/length(brl),length(brl)))

#Por lo tanto: Rechazamos H0

#Tomamos las proporciones teóricas
esperados<-dbinom(x = 0:size, size = size, prob = prob)
esperados<-c(probs[1:7],sum(probs[8:27]))
sum(esperados)
